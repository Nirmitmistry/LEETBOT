...
RERANKER_THRESHOLD      minimum score to keep a chunk  (default: -5.0 for
                        local cross-encoder logits; 0.0 for Cohere 0-1 scores)
...
def _rerank_sync(
    ...
    # 3. Sort descending by score, then cap at top_n
    scored.sort(key=lambda x: x[0], reverse=True)
    top = scored[:top_n]

    # 4. Lost-in-the-middle reorder:
    #    strongest → position 0, second-strongest → position -1, rest fill middle
    #    This exploits the tendency of LLMs to attend more to the start/end of context.
    if len(top) <= 2:
        reordered = [doc for _, doc in top]
    else:
        # Interleave: even indices from the front, odd from the back
        reordered: list[Document] = [None] * len(top)  # type: ignore[assignment]
...
async def retrieve_and_rerank(
    ...
    *,
    ...
    Parameters
    ----------
    query        : The user's query / problem text used for both retrieval and reranking.
    ...
    chroma_filter: Metadata filter dict passed directly to Chroma (e.g. ``{"hint_stage": 0}``).
    ...
    # --- Step 1: Dense retrieval from Chroma (runs in the same event loop — Chroma
    #             client is sync but very fast, no heavy CPU work involved) -----------
    try:
    ...
    # --- Step 2 + 3 + 4: Rerank in a thread-pool so CPU work doesn't block the loop --
